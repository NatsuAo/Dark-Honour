...
public class ChatWnd : WindowRoot 
{
    ...
    private void RefreshUI()
    {
        if (chatType == 0)
        {
            ...
            SetSprite(imgWorld, PathDefine.SelectedBtn);
            SetSprite(imgGuild, PathDefine.UnSelectedBtn);
            SetSprite(imgFriend, PathDefine.UnSelectedBtn);
        }
        else if (chatType == 1)
        {
            SetText(txtChat, "尚未加入公会");
            SetSprite(imgWorld, PathDefine.UnSelectedBtn);
            SetSprite(imgGuild, PathDefine.SelectedBtn);
            SetSprite(imgFriend, PathDefine.UnSelectedBtn);
        }
        else if (chatType == 2)
        {
            ...
            SetSprite(imgWorld, PathDefine.UnSelectedBtn);
            SetSprite(imgGuild, PathDefine.UnSelectedBtn);
            SetSprite(imgFriend, PathDefine.SelectedBtn);
        }
    }
    ...
    public void ClickSendBtn()
    {
        if (!canSend)
        {
            GameRoot.AddTips("世界聊天消息每5秒钟才能发送一条");
            return;
        }
